...
from hyper.utils.geo_manage import gen_map
from hyper.utils.load_shodan import load_file
...
from .forms import ScanForm, RenameScanForm, CreateAssetGroup, AddAssetForm, DeleteAssetForm, AssetScanForm, DocumentForm
...
class DashboardInfoView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['ip_num'] = len(list_devices())
        context['data'] = list_devices()
        return context
...
class GeoView(LoginRequiredMixin, TemplateView):
    template_name = "dashboard/geo/geo.html"
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)            
        return context
    def post(self, request, **kwargs):
        context = self.get_context_data(**kwargs)
        gen_map()
        return redirect('/geo/map')

class GeoMap(LoginRequiredMixin, TemplateView):
    template_name = "dashboard/geo/index.html"
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)            
        return context

class ShodanUpload(LoginRequiredMixin, TemplateView):
    template_name = "dashboard/upload/upload.html"
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)   
        form = DocumentForm()
        context["form"] = form         
    ...
    def post(self, request, **kwargs):
        ...
        if form.is_valid():
            my_file = form.save()
            load_file(my_file.document.path)
            return redirect('/')
        else:
            form = DocumentForm()
            context["form"] = form
...
geo_view = GeoView.as_view()
geo_map_view = GeoMap.as_view()
shodan_upload_view = ShodanUpload.as_view()